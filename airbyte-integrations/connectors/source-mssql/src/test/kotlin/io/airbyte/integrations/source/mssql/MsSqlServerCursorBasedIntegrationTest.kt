...
class MsSqlServerCursorBasedIntegrationTest {
    ...
    fun testCursorBasedRead() {
        ...
        assertEquals(dbContainer.schemaName, lastStreamStateFromRun1.get("stream_namespace").asText())
        ...
        connectionFactory.get().use { connection: Connection ->
            ...
            connection.createStatement().use { stmt: Statement ->
                stmt.execute("INSERT INTO ${dbContainer.schemaName}.$tableName (k, v) VALUES (3, 'baz-ignore')")
                stmt.execute("INSERT INTO ${dbContainer.schemaName}.$tableName (k, v) VALUES (13, 'baz-ignore')")
                stmt.execute("INSERT INTO ${dbContainer.schemaName}.$tableName (k, v) VALUES (30, 'baz')")
            }
        }
    ...
    fun testWithV1State() {
        ...
        assertEquals(actual=recordMessagesFromRun1.size, expected=1, message = recordMessagesFromRun1.toString())
    }
    ...
    companion object {
        ...
        val dbContainer: MsSqlServercontainer = MsSqlServerContainerFactory.shared(MsSqlServerImage.SQLSERVER_2022)
        ...
        fun provisionTestContainer(targetConnectionFactory: JdbcConnectionFactory) {
            ...
            targetConnectionFactory.get().use { connection: Connection ->
                connection.isReadOnly = false
                connection.createStatement().use { stmt: Statement ->
                    stmt.execute("CREATE TABLE ${dbContainer.schemaName}.$tableName(k INT PRIMARY KEY, v VARCHAR(80))")
                }
                connection.createStatement().use { stmt: Statement ->
                    stmt.execute(
