...
class MysqlSourceConfigurationTest {
    ...
    lateinit var factory:
        SourceConfigurationFactory<MsSqlServerSourceConfigurationSpecification, MsSqlServerSourceConfiguration>
    ...
    fun testParseJdbcParameters() {
        val pojo: MsSqlServerSourceConfigurationSpecification = pojoSupplier.get()
    ...
    fun testParseConfigFromV1() {
        ...
        Assertions.assertEquals(config.realHost, "localhost")
        Assertions.assertEquals(config.realPort, 12345)
        Assertions.assertEquals(config.namespaces, setOf("dbo"))

        Assertions.assertEquals(config.jdbcProperties["user"], "FOO")
        Assertions.assertEquals(config.jdbcProperties["password"], "BAR")
        //Assertions.assertEquals(config.jdbcProperties["sslMode"], "required")
        Assertions.assertTrue(config.incrementalReplicationConfiguration is MsSqlServerCdcIncrementalReplicationConfiguration)

        val cdcCursor = config.incrementalReplicationConfiguration as MsSqlServerCdcIncrementalReplicationConfiguration

        Assertions.assertEquals(cdcCursor.initialWaitingSeconds, 301)
