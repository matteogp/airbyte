...
class MsSqlServerSpecIntegrationTest {
    ...
    fun testSyncEmptyTable() {
        ...
        CliRunner.source("discover", config).run()
        println("SGX catalogString=${ObjectMapper().writerWithDefaultPrettyPrinter().writeValueAsString(configuredCatalog)}")

        val readOutput: BufferingOutputConsumer = CliRunner.source("read", config, configuredCatalog, listOf()).run()
        println("SGXX messages=${ObjectMapper().writerWithDefaultPrettyPrinter().writeValueAsString(readOutput.messages())}")
    }
}
